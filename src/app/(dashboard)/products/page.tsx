    ...
    <div className="animate-fade-in-up">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-dark-400 mt-1">Escolha um produto para gerar sua arte.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {filtered.map((product) => (
            <Link key={product.id} href={`/generate?product=${product.id}`}
              ...
              <div className="p-3">
                ...
                {product.category && <p className="text-[11px] text-dark-500 mt-0.5">{(product.category as Category).name}</p>}
              </div>
            </Link>
          ))}
        </div>
