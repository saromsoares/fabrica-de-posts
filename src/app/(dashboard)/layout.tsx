...
import { AdminTestModeBanner } from '@/components/admin/AdminTestModeBanner';
...
// Navegação do ADMIN (padrão — acesso total)
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Modo de teste do Super Admin
  const { testRole, exitTestMode } = useAdminTestMode();
  ...
  const handleLogout = async () => {
    exitTestMode(); // Limpar modo de teste ao sair
    await supabase.auth.signOut();
    router.push('/login');
    router.refresh();
  };

  // Determinar o role efetivo: se admin está em modo de teste, usa o testRole
  const effectiveRole = (() => {
    if (!profile) return 'lojista';
    if (isAdminRole(profile) && testRole) return testRole;
    return profile.role;
  })();

  const getNavItems = (): NavItem[] => {
    if (!profile) return lojistaNav;
    // Admin em modo de teste: mostrar nav do role testado
    if (isAdminRole(profile) && testRole) {
      return testRole === 'fabricante' ? fabricanteNav : lojistaNav;
    }
  ...
  // Configuração visual do role (considera modo de teste)
  const roleConfig = profile ? (() => {
    if (isAdminRole(profile) && testRole === 'fabricante') {
      return { label: 'Modo Teste — Fabricante', color: 'text-blue-400', icon: Factory, accent: 'bg-blue-600/10 border-blue-500/20 text-blue-400' };
    }
    if (isAdminRole(profile) && testRole === 'lojista') {
      return { label: 'Modo Teste — Lojista', color: 'text-brand-400', icon: Store, accent: 'bg-brand-600/10 border-brand-500/20 text-brand-400' };
    }
    if (effectiveRole === 'fabricante') {
      return { label: 'Painel Fabricante', color: 'text-blue-400', icon: Factory, accent: 'bg-blue-600/10 border-blue-500/20 text-blue-400' };
    }
    if (isAdminRole(profile)) {
      return { label: 'Painel Admin', color: 'text-purple-400', icon: Shield, accent: 'bg-purple-600/10 border-purple-500/20 text-purple-400' };
    }
    return { label: 'Painel Lojista', color: 'text-brand-400', icon: Store, accent: 'bg-brand-600/10 border-brand-500/20 text-brand-400' };
  })() : null;

  return (
    <div className="min-h-screen flex flex-col bg-dark-950 text-white">
      {/* Banner de modo de teste — visível apenas para admins em modo de teste */}
      {profile && isAdminRole(profile) && testRole && (
        <AdminTestModeBanner testRole={testRole} onExit={exitTestMode} />
      )}
      ...
      <div className="flex flex-1 min-h-0">
        ...
        >
          ...
          <nav className="flex-1 py-4 px-3 space-y-0.5 overflow-y-auto" aria-label="Menu principal">
            ...
            {/* Admin section — só visível quando não está em modo de teste */}
            {profile && isAdminRole(profile) && !testRole && (
              <>
